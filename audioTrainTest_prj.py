...
def randSplitFeatures(features, per_train):
    ...
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=1 - per_train, random_state=42)
...
def trainSVM_RBF(featuresX, featuresY, Cparam):
    '''
    Train a multi-class probabilitistic SVM classifier.
    Note:     This function is simply a wrapper to the sklearn functionality for SVM training
              See function trainSVM_feature() to use a wrapper on both the feature extraction and the SVM training (and parameter tuning) processes.
...
def trainRandomForest(featuresX, featuresY, n_estimators):
    '''
...
def trainGradientBoosting(featuresX, featuresY, n_estimators):
    '''
...
def featureAndTrain(list_of_dirs_train, list_of_dirs_test, mt_win, mt_step, st_win, st_step,
                    classifier_type, model_name,
                    compute_beat=False, perTrain=0.90):
    '''
    This function is used as a wrapper to segment-based audio feature extraction and classifier training.
    ARGUMENTS:
        list_of_dirs:        list of paths of directories. eachch directory contains a signle audio class whose samples are stored in seperate WAV files.
    ...
    [features_train, classNames_train, filenames_train] = aF.dirsWavFeatureExtraction(list_of_dirs_train, mt_win,
                                                                                     mt_step, st_win, st_step,
                                                                                     compute_beat=compute_beat)

    [features_test, classNames_test, filenames_test] = aF.dirsWavFeatureExtraction(list_of_dirs_test, mt_win, mt_step,
                                                                                   st_win, st_step,
                                                                                   compute_beat=compute_beat)
    ...
    n_feats_train = features_train[0].shape[1]
    feature_names_train = [" Train features" + str(d + 1) for d in range(n_feats_train)]

    n_feats_test = features_test[0].shape[1]
    feature_names_test = [" test features" + str(d + 1) for d in range(n_feats_test)]

    writeTrainDataToARFF(model_name, features_train, classNames_train, feature_names_train)

    for i, f in enumerate(features_train):
        if len(f) == 0:
            print("trainSVM_feature train ERROR: " + list_of_dirs[i] + " folder is empty or non-existing!")
            return

    for i, f in enumerate(features_test):
        if len(f) == 0:
            print("trainSVM_feature test ERROR: " + list_of_dirs[i] + " folder is empty or non-existing!")
            return
    ...
    bestParam = evaluateclassifier(features_train, features_test, classNames_train, classNames_test, 100, classifier_type,
                                   classifier_par, 0, perTrain)

    print("Selected params: {0:.5f}".format(bestParam))

    C = len(classNames_train)
...
def featureAndTrainRegression(dir_name, mt_win, mt_step, st_win, st_step,
    ...
    for c in CSVs:  # for each CSV
        ...
        regression_labels.append(numpy.array(
            cur_regression_labels))  # cur_regression_labels is the list of values for the current regression problem
...
def evaluateclassifier(features_train, features_test, class_names_train, class_names_test, n_exp, classifier_name,
    ...
    for Ci, C in enumerate(Params):
        ...
        for train, test in kf.split(features_norm_train):
            x_train, x_test, y_train, y_test = features_norm_train[train], features_norm_test[test], n_classes_train[
                train], n_classes_test[test]
    ...
    print("\t\t, end=""")
    for i, c in enumerate(class_names_train):
        if i == len(class_names_train) - 1:
    ...
    if parameterMode == 0:  # keep parameters that maximize overall classification accuracy:
        ...
        printConfusionMatrix(cms_all[best_ac_ind], class_names_test)
        return Params[best_ac_ind]
    elif parameterMode == 1:  # keep parameters that maximize overall f1 measure:
        print("Confusion Matrix:")
        printConfusionMatrix(cms_all[best_f1_ind], class_names_test)
...
def fileClassification(inputFile, model_name, model_type):
    ...
    # feature extraction:
    [mt_features, s, _] = aF.mtFeatureExtraction(x, Fs, mt_win * Fs, mt_step * Fs, round(Fs * st_win),
                                                 round(Fs * st_step))
