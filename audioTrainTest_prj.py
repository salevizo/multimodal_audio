...
def featureAndTrain(list_of_dirs_train,list_of_dirs_test,mt_win, mt_step, st_win, st_step, 
    ...
    bestParam = evaluateclassifier(features_train,features_test ,classNames_train, classNames_test,100, classifier_type, classifier_par, 0, perTrain)
...
def evaluateclassifier(features_train,features_test,class_names_train,class_names_test, n_exp, classifier_name, Params, parameterMode, perTrain=0.90):
    '''
    ARGUMENTS:
        features:     a list ([numOfClasses x 1]) whose elements containt numpy matrices of features.
                each matrix features[i] of class i is [n_samples x numOfDimensions]
    ...
    for Ci, C in enumerate(Params):
        ...
        for e in range(n_exp):
            ...
            for c1 in range(n_classes_test):
                n_test_samples = len(features_norm_test[c1])
    ...
    print("\t\t, end=""")
    for i, c in enumerate(class_names_test):
        if i == len(class_names_test)-1:
